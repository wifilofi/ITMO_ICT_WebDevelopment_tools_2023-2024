...
def parse(url: str):
    try:
        ...
        # Сохранение данных в базу данных (пропущено для краткости)
...
6. **Dockerfile**:
   - Создайте Dockerfile для упаковки FastAPI приложения и приложения с паресером. В Dockerfile укажите базовый образ, установите необходимые зависимости, скопируйте исходные файлы и определите команду для запуска приложения.
...
7. **Создание Docker Compose файла**:
   - Напишите docker-compose.yml для управления оркестром сервисов, включающих FastAPI приложение, базу данных и парсер данных. Определите сервисы, укажите порты и зависимости между сервисами.
...
1. **Создайте эндпоинт в FastAPI для вызова парсера**:
...
#### Зачем нужен вызов парсера из FastAPI через очередь?
...
4. **Создайте эндпоинт для асинхронного вызова парсера**:
   - Добавьте в FastAPI приложение маршрут для асинхронного вызова парсера. Маршрут должен принимать запросы с URL для парсинга, ставить задачу в очередь с помощью Celery и возвращать ответ о начале выполнения задачи.
   - **Зачем**: Это позволит вам запускать парсинг веб-страниц в фоне, что улучшит производительность и пользовательский опыт вашего приложения.
   - Полезные ссылки:
     - [Документация FastAPI: Фоновая задачи](https://fastapi.tiangolo.com/tutorial/background-tasks/)

### Залание на максимальгную оценку: Настройка переодических задач с Celery
